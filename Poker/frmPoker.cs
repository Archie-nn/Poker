...
namespace Poker
{
    public partial class frmPoker : Form
    {
        ...
        private void btnCheck_Click(object sender, EventArgs e)
        {
            ...
            bool isLabelValid = int.TryParse(label1.Text, out int labelValue);

            bool isTextValid = int.TryParse(textBox1.Text, out int textValue);
        ...
        {
            // 1. 使用 int.TryParse 確保輸入的是數字，避免程式崩潰
            bool isLabelValid = int.TryParse(label1.Text, out int labelValue);
            bool isTextValid = int.TryParse(textBox1.Text, out int textValue);

            if (isLabelValid && isTextValid)
            {
                // 2. 進行減法運算
                int result = labelValue - textValue;

                // 3. 將結果轉回字串並賦值給 label1
